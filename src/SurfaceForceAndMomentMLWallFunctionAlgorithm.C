...
// Class Definition
//==========================================================================
// SurfaceForceAndMomentMLWallFunctionAlgorithm - post process sigma_ijnjdS and 
...
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
SurfaceForceAndMomentMLWallFunctionAlgorithm::SurfaceForceAndMomentMLWallFunctionAlgorithm(
  Realm &realm,
    ...
    w_(16)
{
  ...
  vectorTauWall_ = meta_data.get_field<VectorFieldType>(stk::topology::NODE_RANK, "vector_tau_wall");
  ...
  assembledArea_ = meta_data.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "assembled_area_force_moment_ml_wf");
  ...
  if ( NULL == vectorTauWallBip_ )
    throw std::runtime_error("SurfaceForce: vector_tau_wall_bip is not registered; wall bcs and post processing must be consistent");
...
//--------------------------------------------------------------------------
void
SurfaceForceAndMomentMLWallFunctionAlgorithm::execute()
{
  ...
  // local force and MomentMLWallFunction; i.e., to be assembled
  double l_force_moment[9] = {};

  // work force, MomentMLWallFunction and radius; i.e., to be pused to cross_product()
  double ws_p_force[3] = {};
  double ws_v_force[3] = {};
  double ws_t_force[3] = {};
  double ws_moment[3] = {};
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ip = 0; ip < numScsBip; ++ip ) {
        ...
        // compute tauWallBip from vector_tau_wall_bip
        double tauWallBip = 0.0;
        for ( int i = 0; i < nDim; ++i ) {
          tauWallBip += vectorTauWallBip[ip*nDim+i]*vectorTauWallBip[ip*nDim+i];
        }
        tauWallBip = std::sqrt(tauWallBip);

        // extract wall friction velocity
        const double utau = std::sqrt(tauWallBip/rhoBip);
        ...
        double *vectorTauWall = stk::mesh::field_data(*vectorTauWall_, node );
        ...
        // assemble vector tauWall
        for ( int i = 0; i < nDim; ++i ) {
          vectorTauWall[i] += vectorTauWallBip[ip*nDim+i]*aMag/assembledArea;
        }

        // assemble scalar tau wall
        *tauWall += tauWallBip*aMag/assembledArea;
...
//--------------------------------------------------------------------------
void
SurfaceForceAndMomentMLWallFunctionAlgorithm::pre_work()
{
...
//--------------------------------------------------------------------------
void
SurfaceForceAndMomentMLWallFunctionAlgorithm::cross_product(
